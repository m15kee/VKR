...
for retrospective in retrospectives:
    ...
    for name, clf in classifiers.items():
        ...
        metrics['Accuracy'].append(accuracy_score(y_test, predictions_test))
        metrics['Recall'].append(recall_score(y_test, predictions_test))
        metrics['Precision'].append(precision_score(y_test, predictions_test))
        metrics['F-Score'].append(f1_score(y_test, predictions_test))
    ...
    # Calculate additional metrics for test data
    metrics['Accuracy'].append(nn_test_accuracy)
    metrics['Recall'].append(recall_score(y_test, nn_predictions_test))
    metrics['Precision'].append(precision_score(y_test, nn_predictions_test))
    metrics['F-Score'].append(f1_score(y_test, nn_predictions_test))
...
# Select two most accurate methods
accuracies_avg = results_df.mean().sort_values(ascending=False)
top_methods = accuracies_avg.head(4).index[::2]
